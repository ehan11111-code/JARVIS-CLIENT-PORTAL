...
function paletteFor(theme: 'light' | 'dark') {
  return {
    ...
    axis: theme === 'light' ? '#8A8780' : '#8A8780',
    base: theme === 'light' ? '#B9B5AA' : '#6E6B63',
    accent: '#F36C34',
...
export function ChartPanel({ chart, locale, height = 240, title }: { chart: Chart; locale: 'en' | 'ar'; height?: number; title?: string }) {
  ...
  const data = chart.series[0].data.map((pt, i) => {
    const row: Record<string, number | string> = { t: pt.t }
    chart.series.forEach((s) => {
      row[s.key] = s.data[i]?.v ?? 0
    })
    return row
  })
        ...
        {mounted && (
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
              ...
              <Tooltip
                cursor={{ stroke: p.accent, strokeWidth: 1, strokeDasharray: '4 4' }}
              ...
              {chart.series.map((s) => (
                <Area
                  ...
                  type="monotone"
                  dataKey={s.key}
                  stroke={s.highlight ? p.accent : p.base}
                  strokeWidth={s.highlight ? 2.4 : 1.6}
                  fill={`url(#grad-${s.key})`}
                  name={s.label[locale]}
                  activeDot={{ r: 4, fill: s.highlight ? p.accent : p.base, stroke: p.tooltipBg, strokeWidth: 2 }}
                />
              ))}
            </AreaChart>
          </ResponsiveContainer>
