...
import { AssistantTopBarButton } from './assistant/AssistantLauncher'
...
export function TopBar({ breadcrumbs }: { breadcrumbs: { label: string; href?: string }[] }) {
  ...
  const [email, setEmail] = useState<string>('demo@user')
      ...
      <div className="flex items-center justify-between gap-4 px-5 md:px-8 py-3.5">
          ...
          <ol className="flex items-center gap-1.5 text-sm text-muted overflow-hidden">
            {breadcrumbs.map((c, i) => (
              <li key={i} className="flex items-center gap-1.5 truncate">
                {i > 0 && <ChevronRight className="h-3.5 w-3.5 text-muted/60 flip-rtl" strokeWidth={1.6} />}
                <span className={i === breadcrumbs.length - 1 ? 'text-text font-medium' : ''}>{c.label}</span>
              </li>
            ))}
          </ol>
        </nav>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <div className="hidden md:flex items-center gap-2 rounded-full border border-border bg-surface px-3 py-1.5">
            ...
            <span className="text-xs text-muted">{firm.clientName}</span>
          </div>
          <AssistantTopBarButton />
          <NotificationBadge />
          <ThemeToggle />
          <LocaleToggle />
          <button
