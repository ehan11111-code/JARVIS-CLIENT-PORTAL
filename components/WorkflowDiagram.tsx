...
const COL_LABEL: Record<string, { en: string; ar: string }> = {
  source: { en: 'Sources', ar: 'المصادر' },
...
export function WorkflowDiagram({ workflow, title, subtitle }: { workflow: Workflow; title?: string; subtitle?: string }) {
  ...
  const byCol = useMemo(() => {
    const cols: Record<number, typeof workflow.nodes> = { 0: [], 1: [], 2: [], 3: [] }
    workflow.nodes.forEach((n) => {
      cols[COLS[n.kind]].push(n)
    })
    return cols
  }, [workflow.nodes])
          ...
          return (
            <div key={c} className="flex flex-col gap-3">
              <div className="flex items-center gap-2 text-xs text-muted">
                ...
                <span>{COL_LABEL[kind][locale]}</span>
              </div>
              {byCol[c].map((n, ni) => (
                <motion.div
                  key={n.id}
                  ...
                  transition={{ duration: 0.4, delay: ci * 0.1 + ni * 0.05, ease: [0.22, 1, 0.36, 1] }}
                  className={
                    'relative rounded-soft border bg-surface-elev px-3.5 py-3 ' +
                    (kind === 'agent'
                      ? 'border-accent/40 shadow-card'
                      : 'border-border shadow-soft')
                  }
                >
                  <div className="text-xs md:text-sm text-text leading-snug" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
                    {n.label[locale]}
                  </div>
                  {kind === 'agent' && (
                    <span className="absolute -top-1.5 -right-1.5 h-3 w-3 rounded-full bg-accent shadow-soft" aria-hidden />
                  )}
                </motion.div>
              ))}
            </div>
