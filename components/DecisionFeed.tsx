...
const OUTCOME_TONE: Record<string, string> = {
  ...
  rejected: 'bg-bg-soft text-muted',
            ...
            <div className="flex-1 min-w-0">
              ...
              {d.confidence !== undefined && (
                <p className="text-xs text-muted mt-0.5">
                  {(d.confidence * 100).toFixed(0)}% {tCommon('confidence')}
                </p>
              )}
            </div>
            <span className={clsx('shrink-0 inline-flex items-center text-[11px] font-medium rounded-full px-2.5 py-0.5', OUTCOME_TONE[d.outcome])}>
              {t(d.outcome)}
            </span>
          </motion.li>
        ))}
