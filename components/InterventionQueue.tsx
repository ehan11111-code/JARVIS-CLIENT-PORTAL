...
const SEV_TONE: Record<string, { icon: string; bg: string }> = {
  high: { icon: 'text-accent', bg: 'bg-accent-soft' },
  medium: { icon: 'text-warn', bg: 'bg-warn-soft' },
  low: { icon: 'text-muted', bg: 'bg-bg-soft' }
}
...
export function InterventionQueue({
  ...
  const tCtrl = useTranslations('control')
  const [resolved, setResolved] = useState<Record<string, 'approved' | 'override' | 'rejected'>>({})
  const [toast, setToast] = useState<string | null>(null)

  const act = (id: string, kind: 'approved' | 'override' | 'rejected') => {
    setResolved((r) => ({ ...r, [id]: kind }))
    setToast(tSol('actionApplied'))
    ...
    <Panel title={title} subtitle={subtitle} bodyClassName="px-0 pb-0">
      ...
      <ul className="divide-y divide-border">
        {interventions.map((iv) => {
          const done = resolved[iv.id]
          const tone = SEV_TONE[iv.severity]
          return (
            <li key={iv.id} className="px-5 md:px-6 py-4 flex flex-col gap-3 md:flex-row md:items-start md:gap-4">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <span className={clsx('shrink-0 inline-flex items-center justify-center h-8 w-8 rounded-full', tone.bg)}>
                  <AlertCircle className={clsx('h-4 w-4', tone.icon)} strokeWidth={1.6} />
                </span>
                <div className="min-w-0">
                  <p className="text-sm text-text leading-snug">{iv.text[locale]}</p>
                  <p className="text-xs text-muted mt-1">{iv.id}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {done ? (
                  <span className="inline-flex items-center text-xs font-medium text-success bg-success-soft rounded-full px-3 py-1">
                    {done}
                  </span>
                ) : (
                  <>
    ...
    <button
      ...
      className={clsx(
        ...
        primary
          ...
          : 'border border-border bg-surface text-text-soft hover:bg-surface-elev hover:text-text'
      )}
    >
