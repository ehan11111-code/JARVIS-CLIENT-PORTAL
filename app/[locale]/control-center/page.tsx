...
export default function ControlCenterPage() {
  ...
  return (
    <PageShell breadcrumbs={[{ label: tNav('operations') }, { label: tNav('controlCenter') }]}>
      ...
      >
        <div className="inline-flex items-center gap-2 rounded-full bg-success-soft text-success px-3 py-1 text-xs font-medium">
      ...
      <section className="mb-8 md:mb-10">
        <MetricRow kpis={firm.firmKpis} locale={locale} />
      </section>

      <section className="mb-8 md:mb-10">
        <DeptHeatStrip departments={firm.departments} title={t('deptHeat')} subtitle={t('deptHeatSub')} />
      </section>

      <section className="mb-8 md:mb-10 grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
        <ActivityTrail
          events={firm.globalActivity}
          locale={locale}
          title={t('executionLog')}
          subtitle={t('executionLogSub')}
          showDept
          maxHeight={420}
        />
        <InterventionQueue
          interventions={firm.globalInterventions}
          locale={locale}
          title={t('interventionsRequired')}
          subtitle={t('interventionsRequiredSub')}
        />
      </section>

      <section>
        <ChartPanel chart={firm.trend} locale={locale} height={280} />
      </section>
    </PageShell>
  )
}
