...
import { getMecState } from './mec-data'
...
function fmtKpi(value: number, format: 'int' | 'pct' | 'sar' | 'ms' | 'hrs') {
  switch (format) {
    case 'int':
      return value >= 1000 ? value.toLocaleString('en-US') : String(Math.round(value))
    case 'pct':
...
function buildSolution(deptSlug: string, seed: SolutionSeed): Solution {
  const r = rng(`${deptSlug}:${seed.slug}`)
  ...
  return {
    ...
    kpis: buildKPIs(r, seed.kpiSeeds),
...
export function getFirmState(): FirmState {
  ...
  const trend: Chart = {
    ...
    series: [
      buildChartSeries(r, 'decisions', { en: 'Decisions', ar: 'قرارات' }, 620, 80),
...
function buildNotifications(departments: Department[]): Notification[] {
  ...
  for (const dept of departments) {
    ...
    if (dept.solutions.length > 0) {
      ...
      list.push({
        ...
        ts: featured.lastRun ? new Date(Date.now() - 1000 * 60 * 60 * (1 + Math.random() * 6)).toISOString() : new Date().toISOString(),
  ...
  list.push({
    ...
    title: {
      en: 'Weekly executive briefing is ready in the Leadership module.',
      ar: 'الإيجاز التنفيذي الأسبوعي جاهز في وحدة القيادة.'
    },
    ts: new Date(Date.now() - 1000 * 60 * 60 * 5).toISOString(),
    link: '/departments/management/executive-briefings',
    read: false
  })

  // MEC: high-severity crisis items become notifications in the global inbox
  try {
    const mec = getMecState()
    for (const n of mec.news.filter((x) => x.severity >= 4).slice(0, 6)) {
      const type: NotificationType = n.severity === 5 ? 'urgent' : 'attention'
      list.push({
        id: `MEC-${n.id}`,
        type,
        deptSlug: 'meat-intel',
        deptName: { en: 'Meat Intel', ar: 'استخبارات اللحوم' },
        title: n.headline,
        body: n.summary,
        ts: n.published_at,
        link: '/meat-intel/crisis-feed',
        read: false
      })
    }
  } catch {
    // mec module optional — ignore
  }
...
export function getDepartment(slug: string): Department | undefined {
  return getFirmState().departments.find((d) => d.slug === slug)
}

export function getSolution(deptSlug: string, solSlug: string): { dept: Department; solution: Solution } | undefined {
